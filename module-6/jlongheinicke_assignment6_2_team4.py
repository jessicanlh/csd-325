#Jessica Long-Heinicke Team 4 Assignment 6.2 11.24.24
    ...
    while True:  # Main program loop.
        ...
        for x in range(forest['width']):
            for y in range(forest['height']):
                ...
                #adding water condition
                if forest[(x, y)] == WATER:
                    nextForest[(x, y)] = WATER  #water cannot change, remains constant
...
def createNewForest():
    ...
    #Add water feature roughly at the center
    lake_width = WIDTH // 4
    ...
    for x in range(WIDTH):
        for y in range(HEIGHT):
            if lake_start_x <= x < lake_start_x + lake_width and lake_start_y <= y < lake_start_y + lake_height:
                forest[(x, y)] = WATER #Place water in this region
    ...
    for y in range(forest['height']):
        for x in range(forest['width']):
            if forest[(x, y)] == TREE:
                ...
                bext.fg('red')
                print(FIRE, end='')
            elif forest[(x, y)] == WATER: #adding visual for water
                bext.fg('blue')
                print(WATER, end='')
